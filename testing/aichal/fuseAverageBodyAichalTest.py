...
def fuse(cmu_idx, aichal_idx):
    ...
    cmu_name = 'pred_cmu_test_{}.json'.format(cmu_idx)
    aichal_name = 'pred_cmu_aichal_v2_test_{}.json'.format(aichal_idx)
    ...
    with open('pred_fuse_averageBody_aichal_test_{}_{}.json'.format(cmu_idx, aichal_idx)
            , 'w') as f:
        json.dump(pred, f)
