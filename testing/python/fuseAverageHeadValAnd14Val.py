...
def compute_dis(kps_cmu, kps_aichal):
    ...
    for j, key in enumerate(kps_cmu):
        ...
        mask[12] = mask[13] = True
        visible  = np.logical_and(visible, mask)
...
def fuse(cmu_idx, aichal_idx):
    ...
    aichal_name = 'pred_cmu_aichal_val_{}.json'.format(aichal_idx)
    ...
    with open('pred_fuse_averageHead_cmu_val{}_aichal_val{}.json'.format(cmu_idx, aichal_idx)
            , 'w') as f:
        json.dump(pred, f)
