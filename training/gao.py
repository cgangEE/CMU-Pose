...
def writeLMDB(datasets, lmdb_path, validation):
	...
	with open('ft.json') as data_file:
		data_this = json.load(data_file)
		data_this = data_this['root']
		data = data + data_this[:1]
	...
	for count in range(numSample):
		...
		cv2.imwrite('mask_all.png', mask_all)
		cv2.imwrite('mask_miss.png', mask_miss)
		print(img.shape, mask_all.shape, mask_miss.shape)
		print(type(mask_miss[0,0]))
		exit(0)
		#print(data[idx])
		...
		print(data[idx]['isValidation'])
		print(data[idx]['numOtherPeople'])
		print(data[idx]['people_index'])
		print(data[idx]['annolist_index'])

		for i in range(len(annolist_index_binary)): # 3,4,5,6
			meta_data[clidx][3+i] = ord(annolist_index_binary[i])
		print(annolist_index_binary)
		...
		kps = np.array(data[idx]['joint_self']).astype(int)
		print(kps.shape)

		for kp in kps:
			cv2.circle(img, (kp[0], kp[1]), 3, (255,0,0), -1)
		cv2.imwrite('kp.png', img)
...
if __name__ == "__main__":
	writeLMDB(['COCO'], '/media/cgangee/X/Realtime_Multi-Person_Pose_Estimation/training/dataset/COCO/lmdb', 1)
